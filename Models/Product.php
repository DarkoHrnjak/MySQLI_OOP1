...
class Product{
    ...
    public static function allProducts(): array{
        ...
        $sql = "SELECT p.*, k.naziv as kategorija FROM produkti p inner join kategorije k on p.kategorijaid = k.id";
    ...
    public static function update($id,$naziv,$kolicina,$cijena,$kategorijaid){
        ...
        $_SESSION["poruka"]="Proizvod naziv {$naziv} uspješno ažuriran!";
        return $stmt->execute();
        }
        catch(mysqli_sql_exception $e){
    ...
    public static function delete($id){
        ...
        try{
            ...
            $stmt->bind_param("i",$id);
            $_SESSION["poruka"]="Proizvod uspješno izbrisana!";
            return $stmt->execute();
        }
        catch(mysqli_sql_exception $e){
            $msg="Greška kod brisana: ".$e->getMessage();
            Redirect::redirectToErrorPage($msg);
            exit;
